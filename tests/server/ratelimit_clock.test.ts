...
// state clean for the rest of the run.
...
describe('rateLimited: per-IP sliding window driven by the injected clock', () => {
  it('flips from not-limited to limited on the (max + 1)th call at a fixed time', () => {
    installClock(1_000_000); // arbitrary fixed T
    const req = fakeReq();
    const max = 3;

    // The first `max` calls are under the limit (rateLimited returns false).
    ...
    expect(rateLimited(req, max)).toBe(true);
  });
  ...
  it('rolls the window so the count resets once the clock passes WINDOW_MS', () => {
    ...
    expect(rateLimited(req, max)).toBe(false); // count 1 @ t=0
    expect(rateLimited(req, max)).toBe(false); // count 2 @ t=0
    expect(rateLimited(req, max)).toBe(true); // count 3 @ t=0, over limit
    ...
    fakeTime = WINDOW_MS;
    expect(rateLimited(req, max)).toBe(false); // count 1 in the new window
    expect(rateLimited(req, max)).toBe(false); // count 2
    expect(rateLimited(req, max)).toBe(true); // count 3, over again
  });
  ...
  it('asserts the exact boundary: an entry counts at WINDOW_MS-1 but ages out at WINDOW_MS', () => {
    ...
    expect(rateLimited(near, max)).toBe(false); // near entry @ t=0
    expect(rateLimited(far, max)).toBe(false); // far entry @ t=0
    ...
    expect(rateLimited(near, max)).toBe(true);
    ...
    fakeTime = WINDOW_MS;
    expect(rateLimited(far, max)).toBe(false);
  });
  ...
  it('keys windows by client IP independently', () => {
    ...
    expect(rateLimited(a, max)).toBe(false);
    expect(rateLimited(a, max)).toBe(true); // a is now limited
    // A different IP has its own window and is unaffected.
    expect(rateLimited(b, max)).toBe(false);
  });
});
...
describe('authThrottled / recordAuthFailure: per-account window driven by the clock', () => {
  ...
  it('keys the throttle per normalized account name', () => {
    ...
    expect(authThrottled('Bob')).toBe(true);
