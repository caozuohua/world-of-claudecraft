...
describe('client HTML shell', () => {
  ...
  it('builds the mobile More drawer on the AAA window-frame grammar with a roomy 3-column tile grid', () => {
    ...
    // All 16 actions ride the grid as tiles, Daily Rewards and the Book of
    // Deeds included in BOTH entries.
    expect([...drawer.matchAll(/class="mobile-btn(?: active)?" id="mobile-/g)]).toHaveLength(16);
    expect(drawer).toContain('id="mobile-daily-rewards"');
    expect(drawer).toContain('id="mobile-deeds"');
    expect(hudMobileCss).toContain(
