...
check('standing -> 200 with rank/total', st.status === 200 && typeof st.data.rank === 'number' && st.data.rank >= 1 && typeof st.data.total === 'number' && st.data.total >= 1);
check('standing without a token -> 401', (await fetch(`${BASE}/api/characters/${characterId}/standing`)).status === 401);
check('standing for a foreign/nonexistent character -> 404', (await api('/api/characters/999999999/standing', { token: tokenA })).status === 404);
...
const cx = await api('/api/characters', { method: 'POST', token: tokenA, body: { name: `${cbase} Zy`, class: 'mage', skin: 0 } });
const cy = await api('/api/characters', { method: 'POST', token: tokenA, body: { name: `${cbase}-Zy`, class: 'rogue', skin: 0 } });
...
check('second of a slug collision gets an id-suffixed slug', py.data.ref === `${expectClash}-${cy.data.id}`);
