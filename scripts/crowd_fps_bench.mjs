...
class Bot {
  ...
  async join() {
    ...
    const reg = await api(
      '/api/register',
      { username: `crowd_${uniq}_${this.i}`, password: 'hunter22', email: `crowd_${uniq}_${this.i}@example.com` },
      undefined,
      xff,
    );
...
async function enterWorld(page) {
  ...
  await api('/api/register', { username: u, password: 'hunter22', email: `${u}@example.com` }, undefined, '172.31.0.1');
