 ...
 * row family. Declared in the IA chat section (onlineOnly) and painted only
 * when main.ts wired the online seam (an offline character has no account).
 * The switch disables (aria-busy) until the persisted state loads; a click
 * flips optimistically, the server echo wins, and a failed write reverts to
 * the last known state. Exported standalone so the round-trip is jsdom-driven
      ...
      for (const section of model.sections) {
        ...
        for (const row of section.rows) {
          ...
          if (row.control === 'deedBroadcasts') {
            // The IA row carries onlineOnly, but the seam is the final truth:
            // main.ts wires it only when an authenticated account exists.
            if (hooks.deedBroadcasts) {
